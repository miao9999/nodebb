...
define('composer', [
	...
	// 给post 数据添加一个 save_id 然后添加到 composer.posts[uuid] 中
	function push(post) {
	...
	// 在哪里调用的？？？？
	// data
	//{"cid":1,"title":"","body":"","tags":[]}
	// 创建 pushdata，添加了一些属性，添加了 save_id
	composer.newTopic = function(data) {
		...
		// {"action":"topics.post","cid":1,"title":"","body":"","tags":[],
		// 	"modified":false,"isMain":true,"project":"cyber",
		// 	"others":{"project":"cyber1","name":"miao","phone":"1239432432"},
		// 	"save_id":"composer:1:cid:1"}
		console.log('composer.js:'+'pushdata:'+ JSON.stringify(pushData))
		console.log('composer.js:'+'data:'+ JSON.stringify(data))
	};
	...
	composer.editPost = function(pid) {
		...
		socket.emit('plugins.composer.push', pid, function(err, threadData) {
			...
			console.log('composer.js:' + 'threaddata:' + JSON.stringify(threadData))
		});
	};
	...
	composer.enhance = function(postContainer, post_uuid, postData) {
		...
		*/
		...
		// 这里注释会显示在浏览器里
		console.log('composer.js:'+'这里能执行吗。。。。')
		console.log('composer.js:'+ JSON.stringify(postContainer))
	...
	function createNewComposer(post_uuid) {
		...
		console.log('composer.js' + 'title:' + title)
	...
	function post(post_uuid) {
		...
		// 产品
		var product = postContainer.find('.cagegory_' + postData.cid).find('select').find('option:selected').text();

		// 版本
		var version = postContainer.find('.versions').find('select').find('option:selected').text()

		// 问题类型
		var question_type = postContainer.find('.question_types').find('select').find('option:selected').text()
		// 所属事业部
		var apartment = postContainer.find('.apartments').find('select').find('option:selected').text()

		// 属性项目
		var project = postContainer.find('.projects').find('select').find('option:selected').text()

		// 邀请回答人
		var answer = postContainer.find('.answers').find('select').find('option:selected').text()
		// 运行环境
		var environment = postContainer.find('.environment').find('input').val()

		// 急切程度
		var degree = postContainer.find('.degree').find('select').find('option:selected').text()

		// 联系人
		var contact = postContainer.find('.contact').find('input').val()

		//  联系方式
		var contact_way = postContainer.find('.contact_way').find('input').val()



		console.log('composer.js---' + 'product:' + product)



		// console.log('composer.js:'+'action:'+ action)
		// console.log('composer.js:'+'postData'+ postData)
		// console.log('composer.js:'+'postContainer:'+ JSON.stringify(postContainer))
		// console.log('composer.js:'+'handleEl:'+ JSON.stringify(handleEl))
		// console.log('composer.js:'+'thumbEl:'+ JSON.stringify(thumbEl))
		// console.log('composer.js:'+'action:'+ action)
		...
		if (uploads.inProgress[post_uuid] && uploads.inProgress[post_uuid].length) {
			...
			if (checkTitle && tags.getTags(post_uuid) && tags.getTags(post_uuid).length < parseInt(config.minimumTagsPerTopic, 10)) {
		...
		console.log('composer.js:'+'action:'+ '是这里吗？')


		var composerData = {};

		// action === 'topics.post'  发贴
		...
		console.log('composer.js' + 'composerData: ' + JSON.stringify(composerData))

		// 这时还没有生成 topic 相关的 json 数据，说明数据还没有提交
		// debugger
		...
		// 在这里发送数据之后返回的数据 data
		// 但这个 data 与 ajaxify 请求回来的数据不一样
		socket.emit(action, composerData, function (err, data) {
		// socket.emit(action, 'composerData', function (err, data) {

			console.log('compose.js:---' + 'ajaxify' + JSON.stringify(ajaxify.data,null,4));
			...
			if (err) {
				...
				// debugger
				console.log('composer.js----' + 'error:' + err.message)
				// err.message: -- > Cannot create property 'title' on boolean 'true'
				...
				if (action === 'topics.post') {
					...
					// console.log('composer.js:' + 'data.slug -----------' + data.slug);

					// 这时已经生成了 json 数据


					ajaxify.go('topic/' + ajaxify.topics[0].slug, undefined, (onComposeRoute || composer.bsEnvironment === 'xs' || composer.bsEnvironment === 'sm') ? true : false);
				} else if (action === 'posts.reply') {
					if (onComposeRoute || composer.bsEnvironment === 'xs' || composer.bsEnvironment === 'sm') {
						...
						if (parseInt(postData.tid, 10) !== parseInt(ajaxify.data.tid, 10)) {
							...
							ajaxify.go('post/' + ajaxify.topics.mainPid);
						}
