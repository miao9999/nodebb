...
define('composer/categoryList', ['categorySelector', 'taskbar'], function(categorySelector, taskbar) {
	...
	categoryList.init = function(postContainer, postData) {
		...
		socket.emit('plugins.composer.getCategoriesForSelect', {}, function(err, categories) {
			...
			categoryList.updateProduct(postContainer,postData)
	...
	// 选择不同的版块（cid）显示相应的产品
	categoryList.updateProduct = function (postContainer, postData) {
		var cid = parseInt(postData.cid);
		var products = postContainer.find('.products .block');

		console.log('categoryList:--' + cid);
		console.log('categoryList:--' + 'product:' + products.length);
